...
2. **What's New extraction** — the top five `## ` sections of the hand-written, plain-language root `WHATS-NEW.md` (house style: `.claude/whats-new-style.md`; falls back to the technical `CHANGELOG.md` only if `WHATS-NEW.md` is somehow missing) are extracted into `public_html/data/whats-new.md`, rendered at `/whats-new` via `includes/markdown_lite.php` (#1583, source switched to `WHATS-NEW.md` in the #1818 plain-language rework). A malformed top section is user-visible here.
