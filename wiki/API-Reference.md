...
| `?page=help` | Help page |
| `?page=whats-new` | What's New — renders `data/whats-new.md`, extracted from the hand-written, plain-language root `WHATS-NEW.md` on every deploy (falling back to the technical `CHANGELOG.md` only if `WHATS-NEW.md` is somehow absent) |
