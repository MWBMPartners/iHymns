...
| **Admin — API Keys / Webhooks** (owner decision Q5) | Management CRUD over `tblApiKeys` and `tblWebhooks`, with a strict **show-once** secret discipline — only the 4 mint/rotate responses (`admin_api_key_create`, the approve-request path, `admin_webhook_create`, `admin_webhook_rotate_secret`) ever carry a plaintext secret; the page's own "reveal an existing secret" action deliberately stays web-only (session, admin-eyes-on). | `admin_api_key_create`, `admin_api_key_toggle`, `api_key_request` (self-service), `admin_webhook_create`, `admin_webhook_rotate_secret`, `admin_webhook_send_test`, `admin_webhook_redrive` |
