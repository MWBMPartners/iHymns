...
- feat(api-platform): **API key usage dashboard + `catalogue:read` keyed reads + dev quickstart** — `/manage/api-keys` now shows per-key **usage today** (`tblApiKeyUsage`, existence-gated) + an editable **per-key rate-limit** modal (Phase B). A new `catalogue:read` scope lets a trusted integrator read the public catalogue at its **own** per-key limit instead of the anonymous IP limit (Phase A, `enforceReadRateLimitKeyed()`; public stays open; gated content untouched). help.php gains a **developer quickstart** (auth header, scopes, `X-RateLimit-*`/`Retry-After`, idempotency, Swagger Try-it) and the OpenAPI documents the scope (Phase C). Reuses the #1066 key infra — no new migration. Strategy: `.claude/api-platform-strategy.md` (Phase D deferred).
