...
> **"Resume the Apple native app build — read `.claude/apple-native-handoff.md`, don't re-plan, autonomously continue (don't ask each step), NO PRs without consent. PHASE 0 IS DONE (#1393–#1401). PHASE 1 IN PROGRESS on `feat/apple-universal`: DONE = #180 song display, #1436 search, #1437 songbooks browse, #183 SOTD+Home, #1398 login/account, #181 favourites+setlists, #182 Settings, #1412 dyslexia+CVD/theme modes, #188 a11y, #187 offline caching+storage UI, #184 audio/PDF/MIDI/MusicXML media (280 tests, 0 lint, macOS BUILD SUCCEEDED). NEXT (build each with Sonnet, Haiku for mechanical, Opus only where flagged; commit per-issue + update the issue + epic #895): nav/UX #185, sharing/deep-links #186, analytics(no-ATT) #189, help/legal/first-run #190; backend auth_apple #1402 + account_delete #1403 (App-Review blockers). Then the Phase-1 dev-team-review gate. Verify each via swift build/test + `xcodebuild -sdk macosx` (simulator can't run locally — CoreSimulator is a point-release behind; macOS build is the proof)."**

## 🔄 PHASE 1 PROGRESS (2026-07-07)
DONE on `feat/apple-universal` (local commits, no PRs): **#180** song display (chords toggle, metadata/credits, per-line translations, works panel, related songs), **#1436** search (+ by-number `CatalogueNumberQuery`, recent searches), **#1437** songbooks browse (official/unofficial badge, series, Collections), **#183** Song-of-the-Day + Home, **#1398** login screen + AccountView, **#181** favourites + setlists (private CRUD + LIVE share, offline queue), **#182** Settings screen (theme + CVD + privacy/analytics consent + account + DEBUG env picker; `IHSettingsStore`/`SettingsViewModel`; app-wide `.preferredColorScheme`; commit `f2ef6c42`), **#1412** dyslexia-friendly reading mode (`IHReadingMode` env + `IHLyricTypography` wiring; light/dark/CVD half via #182; **OpenDyslexic OTF not bundled yet** — `Font.custom` falls back to system + increased spacing, FONT-PENDING note in code; commit `45c9e925`), **#188** a11y (VoiceOver heading-rotor traits + audit of the strong existing coverage; commit `a2a204b0`; deferred high-contrast palette tokens + device VoiceOver sweep → **#1438** `for consideration`), **#187** offline caching + data management (GRDB migration v4 `saved_song` table caching the full `SongDetail`; "Save for Offline" toolbar toggle; `.offline`/`.maintenance` read-fallback → cached copy + "showing saved copy" indicator; "Storage & Offline" Settings screen — size, per-song swipe-remove, remove-all; commit `a458d9d8`; deferred songbook-level "save all" → **#1439** `for consideration`), **#184** media (real backend contract = `SongDetail.media[].streamUrl` → `/song-media/<id>` served by `appWeb/public_html/song-media.php`, gated by dormant `checkContentAccess()`; `kind` = `audio|sheet-music|midi|musicxml`; audio streams w/ HTTP Range, others are attachment downloads). Shipped: AVFoundation audio player (state-machine tested), PDFKit sheet-music viewer, MIDI/MusicXML download+`ShareLink` w/ path-traversal-sanitised filenames, a "Media" card on `SongDetailView`, `UIBackgroundModes: audio` in `project.yml`; commit `862834c2`; deferred → **#1440** offline media caching, **#1441** persistent now-playing bar, **#1442** watchOS/tvOS bg-audio Info.plist keys. **280 tests, 0 lint, macOS BUILD SUCCEEDED.** Each issue closed with evidence; epic #895 ticked.
REMAINING P1: nav/UX #185 · sharing/deep-links #186 (a `DeepLinkRouter` already exists — partial) · analytics #189 (consent flag exists via #182) · help/legal/first-run #190 · backend auth_apple #1402 + account_delete #1403 → Phase-1 dev-team-review gate.
NOTE (stale issue checklists): both #187 and #184 shipped with GitHub checklists describing pre-API-driven mechanics (bundled `songs.json`/BGTaskScheduler for #187; a false "ALL COMPLETE" for #184 with zero AVPlayer/PDFKit code actually present). Both were closed WITH an explanation rather than reinterpreted — worth an owner glance to correct the wording on any remaining P1 issues before building them.
