...
> **"Resume the Apple native app build — read `.claude/apple-native-handoff.md`, don't re-plan, autonomously continue (don't ask each step), NO PRs without consent. PHASE 0 IS DONE (#1393–#1401, working app on `feat/apple-universal`). Continue PHASE 1 (parity core), building each with Sonnet (Haiku for mechanical, Opus only where flagged), committing per-issue + updating the issue + epic #895: song display #180 (enhance SongDetailView — chords toggle, metadata/credits, per-line translations, works panel, related songs), search+search_num, songbooks browse (official/unofficial badge, series, Collections), SOTD #183, a login screen + favourites+setlists #181, Settings #182, a11y #188 + dyslexia toggle #1412, offline #187, audio/PDF/MusicXML #184, nav/UX #185, sharing/deep-links #186, analytics(no-ATT) #189, help/legal #190; backend auth_apple #1402 + account_delete #1403. Verify each via swift build/test + `xcodebuild -sdk macosx` (simulator can't run locally — CoreSimulator is a point-release behind; macOS build is the proof). dev-team-review at the Phase-1 gate."**
