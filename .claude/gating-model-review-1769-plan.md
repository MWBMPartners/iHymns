...
> **P2 DONE. Next: P3 (emitter adoption) — but per the queue, DM-1/DM-2 data migrations + activity logging land in P4-P6 (see below), and the backup-failure #1771 is queued before final docs.** Local `appWeb/.auth/db_credentials.php` (gitignored) → local `ihymns_live` (P1 applied); the equivalence test + golden-capture tool need it moved aside (they self-skip/refuse if a DB is reachable, matching CI).
>
