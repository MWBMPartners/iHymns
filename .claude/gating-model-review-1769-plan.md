...
> **P2 COMPLETE (2026-08-04) — full blueprint in `.claude/gating-p2-design.md`. All DORMANT — no emitter adoption; that's P3.**
> Commit protocol A–G all landed; each mutation-proven; full suite 113/113 CI-parity (getDbMysqli DB-free + IHYMNS_TEST_DSN):
> - ✅ **A** `c86ec62f` — pure code-motion seam: `_contentGatingApplyLegacyCore` / `_contentGatingMediaAllowedLegacyCore` extracted (existing no-op tests prove pure motion). *(deleted in E once the goldens were frozen.)*
> - ✅ **B** `7e7b2a65` — golden capture: `tools/capture-gating-goldens.php` + `tests/php/fixtures/gating-goldens.json` (240 apply + 144 media, DB-free so it matches CI's matrix path).
> - ✅ **C** `185ef7f4` — `includes/licence_registry.php` (the #459 reader + LICENCE_TYPES_FALLBACK + coverage vocab) + `test-licence-registry.php`.
> - ✅ **D** `6c6315bd` — TIER_CAPS optional 5th `enforce` element + `tierCapEnforce()`/`tierCapEnforceShapeValid()` + `gatingCapEnforceColumnExists()` probe + `EnforceJson` into the union (additive; frozen-7 get none) + `test-gating-registry.php` §6 (5-tuple round-trip + frozen-7 mutation sentinel, break→red→restore proven).
> - ✅ **E (the crux)** `c8be01a6` — `access_context.php` (viewer struct) + `access_resolver.php` (`accessApplySong`/`accessMediaAllowed`/`accessResolve` + the triple-locked dead rights-fact branch); the two CG delegates rewritten to lazy-require + route through the pipeline; the legacy cores DELETED; `test-gating-equivalence.php` replays all 384 goldens byte-identically (DB-free) + `test-gating-pipeline-structure.php` + `test-gating-noop.php` (11-13) + `test-media-kind-cap-shared.php` re-pointed. Whole mutation checklist run break→red→restore (M1-M8).
> - ✅ **F** `2d9324d3` — re-pointed the 6 licence-vocab sites onto licence_registry (organisations/restrictions/my-organisations pickers gain mrl+custom; restrictions loses dead `none`; ccli_validator `$licenceToTier`→fallback under live ConfersTier — identity-proven for all 9 inputs, incl. live Part C; 3 schema COMMENTs modernised). ⚠ mrl-confers-no-tier end-state NOT flipped (would change live tiers even gating-off) → **P6 owner decision, filed #1772**.
> - ✅ **G** — docs: `project-rules.md §18.7` (Model-2 pipeline) + §18.4 forward-pointer, CHANGELOG entry, this plan, every new file annotated ELI5+detailed. `RequiresCcli` tier-name-vs-cap inconsistency filed for-consideration **#1773**.
> **P2 DONE.**
>
> **P3 COMPLETE (2026-08-04) — emitter adoption + song.php fork collapse, all dormant. Full blueprint + per-commit SHAs in `.claude/gating-p3-design.md`.** Designed via a sequential Fable pass (orchestrator-verified). A `4a8b9639` seam-extract (diff-proven vs old inline maths, 4608 cells) · B `3ae559b0` 4608-golden capture · C `c9650cd5` the viewer-driven collapse (`songPageGatingDecide`, legacy seam deleted, equivalence replay + `test-gating-pipeline-structure.php` §e, mutation M1–M7) · D `e5c764a2` songbook_export viewer-once hoist · E `c6a2c86a` gated-song cache exclusion (api.php `no-store` + SW `swResponseCacheable`, §f guard). **F DEFERRED → #1777** (checkBulkAccess presence wiring is state-(b)). G docs (project-rules §18.7 P3 para, CHANGELOG) + follow-ups #1774/#1775/#1776/#1777. PHP 114/114 + node 50/50 CI-parity. The song.php quirk (presence ORs sheet/midi too) preserved byte-identically (#1774).
>
> **Next: P4 (admin hub + Rights-panel editor + DM-2 existing-gating→per-song-facts migration + activity logging) → P5 (DM-1 org-licence consolidation migration) → P6 (first-enable + the deferred owner decisions #1772/#1773/#1774/#1775/#1776/#1777).** The backup-failure #1771 is queued before final docs (diagnosis recorded on the issue). Local `appWeb/.auth/db_credentials.php` (gitignored) → local `ihymns_live` (P1 applied); the equivalence tests + capture tools need it moved aside (they self-skip/refuse if a DB is reachable, matching CI).
>
