...
| #1695 notification | App-level. Nothing on `tblSongs`. ⚠️ **CORRECTION (2026-07-31, stage-3 planning):** this row originally read "`PUSH_KINDS` from #1671 has **not landed** (grepped: zero hits)". **It HAD landed** — commit `9fea9427`, hours earlier the same day. The grep returned zero because it searched for a name that has never existed anywhere in this tree: the registry is **`webPushKinds()`** (`includes/web_push.php:138`). *A zero-hit grep for a remembered identifier proves nothing about the feature — only about the spelling.* That is the same class as the `[^;]*?`-truncated-by-a-quoted-semicolon miss in #1704. What IS true: the push chain is cryptographically verified against RFC 8291 §5's own test vector, but **dormant on every docroot** (`webPushConfigured()` false until an operator generates a VAPID keypair) and no push has ever reached a real device. The mechanism that works TODAY is the in-app bell, `notifyUser()` in `includes/notifications.php` (#1638). |
