...
17. **Songs static cache** → `includes/songs_cache.php` (#933 / #932). Read endpoints (`/api?action=songs_json`, `/manage/editor/api?action=load`) serve from a precomputed file at `appWeb/data_share/song_data/songs.json` via `songsCacheServe()` — peak memory <2 MB, gzip / brotli precompressed siblings. Cache regenerated by `save_song`, every bulk-import flow, and the `/manage/data-health` regenerate button. Never call `SongData::exportAsJson()` from a read endpoint — it costs ~140 MB of PHP-array memory.
...
- A read-side endpoint that calls `SongData::exportAsJson()` directly. The corpus rebuild is ~140 MB of PHP-array memory; reads MUST go through `songsCacheServe()` (#933).
