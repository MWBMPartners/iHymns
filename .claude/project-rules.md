...
- **Activity Log error capture (#695).** Admin save handlers that catch `\Throwable` and surface a generic banner ALSO call `logActivityError()` so the failure lands in `tblActivityLog` with `Result='error'`. The viewer at `/manage/activity-log` filters by Result. `error_log()` calls remain as belt-and-braces. Use a stable `admin.<surface>.<action>` verb (`admin.songbooks.save`, `admin.requests.update`, etc.) so the Action filter narrows usefully. Sensitive fields (passwords / tokens) MUST stay out of the Details JSON — see §11 below.
