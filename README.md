...
- **Emphasise Links** (#1984) — an opt-in toggle (Settings → Appearance) that gives in-text links an accent colour and an underline, for anyone who wants links to stand out more than the site's normal understated hover-only styling. Off by default; the underline was added alongside the colour on 2026-08-30 so the opt-in state clears WCAG AA text-contrast in both light and dark theme.
...
- **2026-08-30 security & accessibility audit** — two passes across the day. Security: the Database Setup dashboard's action links gained a CSRF check; the outbound clients for CueRCode, IntApps and Internet Archive now refuse to contact a private/internal address (including one written as a bracketed IPv6 or numeric-IPv4 literal) even if an admin-typed URL points at one; a stored-XSS gap in the publisher page's structured-data block was closed; a licence-expiry check that had only been enforced on one of two code paths no longer lets an expired organisation licence keep granting paid-tier access. Accessibility: a whole-app WCAG 2.1 AA review (0 Critical, 2 High, 8 Medium, 10 Low findings, all fixed) raised colour contrast across admin buttons, badges and small text; five pop-up panels (the keyboard-shortcuts guide, presentation mode, song comparison, the quick song-number picker) now properly trap and return keyboard focus; every song, songbook, tag, musician, publisher, tune and work page sets a real browser-tab title instead of a generic one; the opt-in **Emphasise Links** accessibility option now adds an underline as well as colour; and dozens of smaller labelling gaps were filled in across the admin area and the classic song editor. See [Security](SECURITY.md) for the summary.
