# 📖 iHymns — Help Documentation

> User guides, troubleshooting, and frequently asked questions.

---

## 📚 Contents

| Guide | Description |
| --- | --- |
| [Getting Started](getting-started.md) | How to install and start using iHymns |
| [Searching Songs](searching-songs.md) | How to search by title, lyrics, songbook, and more |
| [Songbooks](songbooks.md) | Browse and navigate the available songbooks |
| [External Links & Works](external-links-and-works.md) | Find songs, songbooks and people elsewhere on the web; understand Works (groupings of the same composition across sources) |
| [Favorites](favorites.md) | Save and manage your favorite songs |
| [Troubleshooting](troubleshooting.md) | Common issues and how to resolve them |
| [FAQ](faq.md) | Frequently asked questions |

---

## 🖥 Platform-Specific Help

- **Web PWA**: Help is accessible via the in-app help menu
- **Apple (iOS/iPadOS/tvOS)**: Help is embedded natively in the app's Settings
- **Android**: Help is embedded natively in the app's Settings

---

## 📬 Need More Help?

If you can't find what you're looking for, please [open an issue](https://github.com/MWBMPartners/iHymns/issues) on GitHub.

---

Copyright &copy; 2026 MWBM Partners Ltd. All rights reserved.
