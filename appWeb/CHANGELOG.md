...
- **Per-organisation logo uploads for Print Templates** (#1830) — a church can upload its logo (SVG preferred, PNG/APNG also accepted) in any of ten brand-guide shapes from `/manage/organisations` or `/manage/my-organisations`. SVG uploads pass through a brand-new, dedicated hardened sanitiser (`includes/svg_sanitizer.php`, XXE-safe default-deny XML rebuild) before storage; logos are served by a new `org-logo.php` endpoint and never inlined into any page. A new `logo` block in the Print Template editor picks a specific brand shape or "Automatic" (best available, via a documented fallback ladder). Print-only scope for this PR; app header/projector/OG-image surfaces are a deferred follow-up.
...
- **IA-reconcile Phase 1** (#94) — a read-only archive.org OCR audit tool at `/manage/ia-reconcile`; scores a scan against a songbook and reports the gap list, never writing song content.
...
- **Live Follow session length + extend, plus cross-channel error** (#1770 / #1792 / #1798) — a persistent host bar, leader-idle auto-close, host-CCLI unlock, an external presentation-app driver, a clearer wrong-environment message, and a declared session length with live / on-behalf Extend.
- **QR codes now served by `/qr.php`** (CueRCode-backed, owner directive) — the vendored client-side QR encoder was removed; the Service-Projection join QR and the print QR block both point an `<img>` at the same-origin endpoint.
- **Set-list collaboration via share-link, client half** (#1791) — the Share dialog on a setlist's detail page now mints THREE kinds of grant: a view link (unchanged), a new anonymous-capable **edit link** (per-link "who can edit" — anyone-with-the-link or signed-in-only — plus an optional "show my name" byline, with an Active-links list and per-link Revoke), and the existing email-invite Collaborators panel. The public shared page renders an editable reorder/remove surface when the server grants `canWrite`, downgrading to a sign-in prompt on a `401 signin_required` refusal. Org admins gained a "Set-list edit links" preference on `/manage/my-organisations`. See the root CHANGELOG for the full write-up.
- **Musician merge/dedup follow-ups** (#1800, follow-on to #1785/#1796) — `credit_search`'s third hand-typed credit-table list collapsed onto the shared `MUSICIAN_CREDIT_ROLE_TABLES`; `musicianMergeExecute()` now COALESCE-fills the survivor's empty Biography/MusicBrainz-id/Disambiguation/birth-death-date fields from the source before deleting it (never overwriting a non-empty target value); a soft, non-blocking "possible duplicate — view?" hint on `/manage/musicians`' create form catches an exact-fold name match before the insert. Unifying `/manage/musician-duplicates` with `/manage/musicians-bulk-promote` was assessed against the #1215 precedent and deferred — the two pages' candidate universes (unregistered names vs. registry-row pairs) don't naturally absorb into one surface.
