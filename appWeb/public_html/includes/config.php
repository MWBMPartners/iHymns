...
define('APP_CONFIG', [
    ...
    'libraries' => [
        ...
        /* QR Code Generator 2.0.4 — kazuhikoarase/qrcode-generator (MIT).
         * Renders the congregant join QR on manage/service-projection.php
         * (#1339, the "buildable half" — CLAUDE.md rule #26 / #1587).
         *
         * ELI5: this is the tiny library that turns the join URL + rotating
         * code into the scannable black-and-white square on the projector.
         *
         * Chosen over a hand-rolled encoder: QR encoding (Reed-Solomon error
         * correction, mask-penalty scoring, exact format/version bit
         * placement) is easy to get APPROXIMATELY right and hard to get
         * EXACTLY right, and a QR that scans on one phone but not another
         * is worse than no QR at all. This is the ~decade-old, MIT,
         * dependency-free reference implementation (no framework, no build
         * step) — same pinned + SRI + vendored shape as the Bootstrap /
         * jQuery / Swagger UI entries above (#354 / #526 / #1587).
         *
         * NOTE: like PDF.js above, this is loaded via dynamic import() of
         * the ES module build (`dist/qrcode.mjs`) from
         * service-projection.php's own `<script type="module">` — browsers
         * do not support the `integrity` attribute on dynamic import() (SRI
         * only applies to `<script src>` / `<link>` fetches), so `js_sri`
         * is recorded for audit/tests only and is NOT enforced at runtime.
         * A failed CDN import falls through to the vendored copy in code,
         * and a failure of BOTH degrades to the plain typed code — never a
         * blank box (see service-projection.php's renderQr()).
         *
         * The hash below was computed from the published npm tarball
         * (`qrcode-generator-2.0.4.tgz`'s `dist/qrcode.mjs`), which is
         * byte-for-byte what jsDelivr serves at this exact npm path (the
         * same equivalence the Swagger UI entry above documents) —
         * jsDelivr itself was unreachable from the dev sandbox that pinned
         * this (the outbound proxy blocks it; the npm registry does not),
         * so the hash was verified against the npm registry tarball
         * instead. tests/test-qr.js re-derives it from the same tarball
         * (or the vendored copy, once downloaded) and asserts equality,
         * plus independently proves the file it names actually produces a
         * spec-correct, decodable QR code (not just "a file exists").
         * https://developer.mozilla.org/en-US/docs/Web/Security/Subresource_Integrity */
        'qrcodegen' => [
            'version'    => '2.0.4',
            'js_cdn'     => 'https://cdn.jsdelivr.net/npm/qrcode-generator@2.0.4/dist/qrcode.mjs',
            'js_sri'     => 'sha384-xHtsZTi0cOhTiha4kloGDUR5e5A8ixkKsF1Mmpqq3tkDpAhkA4YOKIDua/Tc0Ecd',
            'js_local'   => 'vendor/qrcode-generator/qrcode.mjs',
        ],
    ],
