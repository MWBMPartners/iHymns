...
export const PRINT_BLOCK_TYPES = {
    ...
    qr:          { label: 'QR code',         options: { size: 'md' } },            /* #1767 R — QR of the permalink, rendered as an <img> to the CueRCode-backed /qr */
    logo:        { label: 'Organisation logo', options: { kind: 'auto', size: 'md', align: 'center' } },  /* #1830 — the org's uploaded branding, rendered as an <img> to /org-logo */
    ...
    switch (block.type) {
        ...
        case 'qr': {
            /* #1767 R — a scannable QR of the public permalink. The QR image is
               ...
               same-origin /qr endpoint (owner directive 2026-08-05 — QR via
               ...
               access" rule matches on the RAW request text, which never sees a
               later rewrite), a routing bug that made every printed QR block
               dead until the /qr alias was added (see .htaccess, rules #33/#38). */
            const origin = (typeof window !== 'undefined' && window.location) ? window.location.origin : '';
            const pid = song.publicId || song.id || '';
            if (!pid) { return ''; }
            const url = origin + '/song/' + encodeURIComponent(pid);
        ...
        case 'logo': {
            ...
            /* ELI5/WHY: "/org-logo", not "/org-logo.php" — same routing-bug
               fix as the QR block above (rules #33/#42): the .htaccess block
               rule fires on the raw request text before any rewrite runs, so
               a literal ".php" in this src has never been reachable. */
            const origin = (typeof window !== 'undefined' && window.location) ? window.location.origin : '';
            const src = origin + '/org-logo?org=' + encodeURIComponent(stash.orgId)
...
/* #1767 R / owner directive 2026-08-05 — QR is now generated by the CueRCode API
   server-side and served by the same-origin /qr endpoint; renderBlock('qr')
 ...
 * same-origin src (`window.location.origin + '/org-logo?…'`) because the
 ...
 * 'layout' profile, whose `img_src` allow-list admits ONLY the root-relative
 * `#^/qr\?#` / `#^/org-logo\?#` shape (an absolute URL matches neither and is
 * dropped) — and `pdf_renderer.php`'s `_pdfInlineQrImage()` /
 * `_pdfInlineOrgLogo()` key on those same `^/…\?` prefixes. So an
 ...
 *
 * The `\.php` extension is deliberately ABSENT from the endpoint group: both
 * endpoints are reached only via their extensionless `.htaccess` alias
 * (`^qr$`/`^org-logo$` → `qr.php`/`org-logo.php`, the /og-image precedent,
 * rules #33/#38/#42) — a literal `.php` in the raw request line 404s at the
 * "block direct PHP access" rule before any PHP ever runs, so this function
 * (and the sanitiser/inliner it exists to feed) must never reintroduce it.
 *
 ...
 * @returns {string} The bodyHtml with same-origin /qr + /org-logo img
...
export function pdfRelativiseServerImgSrc(html, origin) {
    ...
    return html.replace(
        new RegExp('(src=")' + escOrigin + '(/(?:qr|org-logo)\\?)', 'g'),
        '$1$2'
    );
}
...
export async function openSongPrintDialog(app) {
    ...
    /* #1767 R — a `qr` block renders as an <img> to the same-origin /qr
