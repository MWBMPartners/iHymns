...
/* _songArtistsTableExists() — the tblSongArtists existence probe (#587) — moved
   to save_song_core.php (#1200), its only remaining consumer, when the whole-song
   save was extracted into the shared editorSaveSongCore(). It is defined there
   under a function_exists() guard, so it is available to BOTH editor APIs without
   a redeclaration collision. */
...
switch ($action) {
     ...
     * ----------------------------------------------------------------- */
    case 'save_song':
        /* #1200 — the whole-song save now lives in the shared editorSaveSongCore()
           so the legacy api.php and the v2 api2.php run the SAME primary save path
           (CLAUDE.md modularity rule — no forked save logic). The core returns the
           HTTP status + JSON body; this endpoint emits it in its house style. */
