            ...
            <main class="col-lg-9">
                ...
                <section id="editor" class="help-section card-admin mb-4">
                    ...
                    <ul>
                        ...
                        <li><strong>ProPresenter 7+</strong> (<code>.pro</code>, <code>.probundle</code>, <code>.proplaylist</code>) — import + export. A bare <code>.pro</code> file is content-sniffed to tell it apart from ChordPro's own <code>.pro</code> extension automatically, so the two can sit in the same folder without sorting first. A <code>.probundle</code> is ProPresenter's own "everything in one file" export &mdash; importing one brings in every song inside it, plus any background video or image a song has attached; exporting builds one back the same way, for a single song or the whole filtered songbook. A <code>.proplaylist</code> (ProPresenter's service running order) imports as a ready-made set list rather than loose songs. Chords and the song's verse order both travel across, the same as ProPresenter 6 above.</li>
