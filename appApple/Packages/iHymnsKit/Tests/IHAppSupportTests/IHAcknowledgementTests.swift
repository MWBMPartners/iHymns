...
// really is flagged as not-yet-bundled rather than silently claimed as
// shipped.
//
...
struct IHAcknowledgementTests {
    ...
    @Test("OpenDyslexic is SIL OFL 1.1 and honestly flagged as not yet bundled")
    func openDyslexicIsFontPendingOFL() {
        ...
        #expect(openDyslexic?.note?.contains("FONT-PENDING") == true)
    }
