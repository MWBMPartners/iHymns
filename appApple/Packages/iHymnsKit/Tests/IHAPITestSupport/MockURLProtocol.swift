...
// comes from EVERY test suite that mutates `requestHandler` marking itself
// `.serialized` (see `IHAPITests/NetworkedAPIClientTests.swift` and
// `IHAuthTests/SessionControllerTests.swift`) — Swift Testing then
// guarantees at most one test in that suite runs at a time, so there is
// never a genuinely concurrent read/write of this static var, only a
// sequential handoff across a `URLSession` queue hop (which itself provides
// the needed memory barrier).
//
// #1398/#1399 UPDATE — originally lived inside `IHAPITests` only. Moved to
// this dedicated, non-test `.target` (mirroring `IHTestFixtures`'s existing
// precedent, `Tests/Fixtures/`) so `IHAuthTests` (`SessionController`'s
// `auth_login`/`auth_logout` tests) and `IHFeaturesTests`
// (`AppRootViewModel`'s catalogue-load tests) can `import IHAPITestSupport`
// and reuse the SAME mock instead of each hand-rolling — or worse,
// copy-pasting — their own. A `.testTarget` cannot itself be depended on by
// another `.testTarget` in SwiftPM, which is exactly why this is a plain
// `.target` under `Tests/` rather than a fourth test target.
import Foundation
...
/// A small, manually-synchronized mutable counter — used by retry/call-count
/// tests to count how many times the mock transport was hit, without
